...
exports.dentist_register = async (req, res) => {
    try {
        ...
        if (await Dentist.findOne({
            ...
            return res.status(400).send({
                error: 'This CRO already exists'
            });


        const createdDentist = await Dentist.create({
            email,
            name,
            telephone,
            date_birth,
        ...
        res.status(400).send({
            error: 'Error in create a new Dentist'
        });
        console.log(err);
    }
};
